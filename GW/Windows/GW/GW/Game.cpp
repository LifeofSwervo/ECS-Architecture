...
void Game::sMovement(void)
{
    ...
    for (const auto& entity : m_entities.getEntities("enemy"))
    {
        if (entity->cShape)
        {
            ...
            Vector2 direction =
            {
                // Subtract player cords with enemy cords
                m_player->cShape->center.x - entity->cShape->center.x,
                m_player->cShape->center.y - entity->cShape->center.y
            };

            float magnitude = sqrt(direction.x * direction.x + direction.y * direction.y);
            if (magnitude > 0)
            {
                // Divide the magnitude by the direction
                direction.x /= magnitude;
                direction.y /= magnitude;
            }

            entity->cShape->center.x += direction.x * entity->cTransform->velocity.x;
            entity->cShape->center.y += direction.y * entity->cTransform->velocity.y;
        }
    }
}
